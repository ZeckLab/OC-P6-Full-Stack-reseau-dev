...
/** Manages comment loading, creation and state for the UI. */
...
export class Comments {
  ...
  /** Loads comments for an article. */
  load(articleId: string) {
    return this.api.getByArticle(articleId).pipe(
      tap({
        next: (response) => {
  ...
  /** Creates a new comment and adds it locally. */
  ...
  /** Sets the comments list directly (used when backend returns comments with the article). */
